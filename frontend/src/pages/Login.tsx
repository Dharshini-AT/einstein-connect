...
const Login = () => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (email === faculty.email) {
      localStorage.setItem("loggedInUser", JSON.stringify({ role: "faculty", ...faculty }));
      navigate("/dashboard");
    } else {
      toast({ title: "Login Failed", description: "Invalid email. Try sureka@einstein.edu", variant: "destructive" });
    }
  };
    ...
    <div className="flex min-h-screen items-center justify-center p-4">
      <Card className="w-full max-w-md border-border bg-card animate-fade-in">
        <CardContent className="pt-8 pb-8 px-8">
          ...
          <p className="text-center text-muted-foreground text-sm mb-6">Faculty Information Portal</p>
